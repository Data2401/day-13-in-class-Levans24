...
my_flights %>% 
  group_by(dest) %>% 
  summarize(avg_delay = mean(arr_delay)) -> dest_delay
glimpse(dest_delay)
glimpse(airports)
```
...
```{r}
avg_dep_delay_names <- flights %>% 
  group_by(carrier) %>% 
  summarise(avg_dep_delay = mean(dep_delay, na.rm = TRUE)) %>% 
  left_join(airlines, by = "carrier") %>% 
  select(name, avg_dep_delay) %>% 
  arrange(desc(avg_dep_delay))
head(avg_dep_delay_names, 1)
...
```{r}
flights %>% 
  left_join(planes %>% select(tailnum, manufacturer, model , year), by = "tailnum")%>% 
  group_by(tailnum, carrier, model, year) %>% 
  summarise(n_flights = n(), avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>% 
  ungroup() %>% 
  filter(n_flights >= 50) %>% 
  mutate(age = 2013 - year) %>% 
  arrange(desc(avg_arr_delay)) %>% 
  slice(1)
  select(tailnum, manufacturer, model, age, avg_arr_delay)
